...
/**
 * A {@link RecordReader} for Avro container files.
 */
public class AvroRecordReader<T> extends RecordReader<AvroKey<T>, NullWritable> {
    ...
    public void initialize(InputSplit split, TaskAttemptContext context) throws IOException, InterruptedException {
        ...
        // set in the job's configuration.
        Schema schema = (new Schema.Parser()).parse(context.getConfiguration().get(Driver.AVRO_SCHEMA));
        reader = new DataFileReader<T>(fileInput, new SpecificDatumReader<T>(schema));

        // Initialize the start and end offsets into the input file. The reader
        // syncs to the first record after the start of the input split.
