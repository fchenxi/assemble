...
import cn.panda.remoting.codec.DefaultCodec;
import cn.panda.remoting.codec.IdleState;
import cn.panda.remoting.concurrent.ServiceThread;
...
public abstract class AbstractRemoting {
    ...
    // 注册的各个RPC处理器
    protected final HashMap<Integer/* request code */, Pair<RemotingProcessor, ExecutorService>>
            processorTable =
    ...
    public AbstractRemoting(final int permitsOneway, final int permitsAsync, ChannelEventListener
    ...
    public void processRequestCommand(final Channel channel, final RemotingCommand cmd) {
        final Pair<RemotingProcessor, ExecutorService> matched = this.processorTable.get(cmd
                .getCode());
        ...
        if (pair != null) {
                    ...
                    try {
                            ...
                            if (response != null) {
                                ...
                                try {
                                    channel.writeAndFlush(response).addListener(new ChannelHandlerListener() {
                                        @Override
                                        public void operationComplete(cn.panda.remoting.Future
                                                                              future)
                                                throws
                                                Exception {
                                            if (!future.isSuccess()) {
                                                LOGGER.error("response to " + RemotingHelper
                        ...
                        if (!RemotingCommandHelper.isOnewayRPC(cmd)) {
                            final RemotingCommand response =
                                    RemotingCommand.createResponseCommand(RemotingProtos
            ...
            try {
                ...
                if (!RemotingCommandHelper.isOnewayRPC(cmd)) {
                    final RemotingCommand response =
                            RemotingCommand.createResponseCommand(RemotingProtos.ResponseCode
                                            .SYSTEM_BUSY.code(),
                                    "too many requests and system thread pool busy, please try " +
                                            "another server");
                    response.setOpaque(cmd.getOpaque());
                    channel.writeAndFlush(response);
                }
            ...
            if (responseFuture.getAsyncCallback() != null) {
                ...
                if (executor != null) {
                    try {
                        executor.submit(new Runnable() {
                            ...
                            public void run() {
                                try {
                                    ...
                                    LOGGER.warn("execute callback in executor exception, and " +
                                            "callback throw", e);
                                }
                            }
                        });
                        ...
                        LOGGER.warn("execute callback in executor exception, maybe executor " +
                                "busy", e);
                    }
                } else {
                    runInThisThread = true;
    ...
    public void processMessageReceived(Channel channel, final RemotingCommand cmd) throws
            Exception {
        if (cmd != null) {
    ...
    public RemotingCommand invokeSyncImpl(final Channel channel, final RemotingCommand request,
        ...
        try {
            ...
            channel.writeAndFlush(request).addListener(new ChannelHandlerListener() {
                @Override
                public void operationComplete(cn.panda.remoting.Future future) throws Exception {
                    if (future.isSuccess()) {
            ...
            if (null == responseCommand) {
                ...
                if (responseFuture.isSendRequestOK()) {
                    throw new RemotingTimeoutException(RemotingHelper.parseChannelRemoteAddr
                            (channel),
                            timeoutMillis, responseFuture.getCause());
                }
                // 发送请求失败
                else {
                    throw new RemotingSendRequestException(RemotingHelper.parseChannelRemoteAddr
                            (channel),
    ...
    public void invokeAsyncImpl(final Channel channel, final RemotingCommand request,
                                final long timeoutMillis, final AsyncCallback asyncCallback)
        ...
        if (acquired) {
            ...
            try {
                channel.writeAndFlush(request).addListener(new ChannelHandlerListener() {
                    @Override
                    public void operationComplete(cn.panda.remoting.Future future) throws
                            Exception {
                        if (future.isSuccess()) {
                            responseFuture.setSendRequestOK(true);
                            return;
                ...
                once.release();
                LOGGER.warn("write send a request command to channel <" + channel.remoteAddress()
                        + "> failed.");
                throw new RemotingSendRequestException(RemotingHelper.parseChannelRemoteAddr
                        (channel), e);
            }
    ...
    public void invokeOnewayImpl(final Channel channel, final RemotingCommand request,
        ...
        if (acquired) {
            final SemaphoreReleaseOnlyOnce once = new SemaphoreReleaseOnlyOnce(this
                    .semaphoreOneway);
            try {
                channel.writeAndFlush(request).addListener(new ChannelHandlerListener() {
                    @Override
                    public void operationComplete(cn.panda.remoting.Future future) throws
                        ...
                        once.release();
                        if (!future.isSuccess()) {
                            LOGGER.warn("send a request command to channel <" + channel
                                    .remoteAddress()
                                    + "> failed.");
                            LOGGER.warn(request.toString());
                        }
    ...
    class RemotingEventExecutor extends ServiceThread {
        private final LinkedBlockingQueue<RemotingEvent> eventQueue = new
                LinkedBlockingQueue<RemotingEvent>();
        ...
        public void putRemotingEvent(final RemotingEvent event) {
            if (this.eventQueue.size() <= MaxSize) {
                ...
                LOGGER.warn("event queue size[{}] enough, so drop this event {}", this.eventQueue
                                .size(),
                        event.toString());
            }
        }
                ...
                try {
                    ...
                    if (event != null) {
                        switch (event.getType()) {
                            case ALL_IDLE:
                                listener.onChannelIdle(IdleState.ALL_IDLE, event.getRemoteAddr(),
                                        event.getChannel());
                                break;
                            case WRITER_IDLE:
                                listener.onChannelIdle(IdleState.WRITER_IDLE, event.getRemoteAddr
                                        (), event.getChannel());
                                break;
                            case READER_IDLE:
                                listener.onChannelIdle(IdleState.READER_IDLE, event.getRemoteAddr
                                        (), event.getChannel());
                                break;
                            case CLOSE:
                            ...
                            case CONNECT:
                                ...
                                break;
                            case EXCEPTION:
                                listener.onChannelException(event.getRemoteAddr(), event
                                        .getChannel());
                                break;
                            default:
                                break;
