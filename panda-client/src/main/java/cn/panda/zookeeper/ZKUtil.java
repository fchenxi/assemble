...
public class ZKUtil {
  ...
  throws KeeperException {
    createAndFailSilent(zkw,
        (ZKUtilOp.CreateAndFailSilent)ZKUtilOp.createAndFailSilent(znode, data));
  }

  private static void createAndFailSilent(ZooKeeperWatcher zkw, ZKUtilOp.CreateAndFailSilent cafs)
  throws KeeperException {
  ...
  throws KeeperException {
    deleteNodeFailSilent(zkw,
      (ZKUtilOp.DeleteNodeFailSilent)ZKUtilOp.deleteNodeFailSilent(node));
  }

  private static void deleteNodeFailSilent(ZooKeeperWatcher zkw,
      ZKUtilOp.DeleteNodeFailSilent dnfs) throws KeeperException {
  ...
  public static void deleteChildrenRecursivelyMultiOrSequential(
    ...
    List<ZKUtilOp> ops = new ArrayList<ZKUtilOp>();
  ...
  private static void processSequentially(ZooKeeperWatcher zkw, List<ZKUtilOp> ops)
    ...
    for (ZKUtilOp op : ops) {
      if (op instanceof ZKUtilOp.CreateAndFailSilent) {
        createAndFailSilent(zkw, (ZKUtilOp.CreateAndFailSilent) op);
      } else if (op instanceof ZKUtilOp.DeleteNodeFailSilent) {
        deleteNodeFailSilent(zkw, (ZKUtilOp.DeleteNodeFailSilent) op);
      } else if (op instanceof ZKUtilOp.SetData) {
        setData(zkw, (ZKUtilOp.SetData) op);
      } else {
        throw new UnsupportedOperationException("Unexpected ZKUtilOp type: "
            + op.getClass().getName());
      }
